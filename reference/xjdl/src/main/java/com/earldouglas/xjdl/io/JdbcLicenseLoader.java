...
public class JdbcLicenseLoader extends LicenseLoader {
	...
	public License readLicense() throws Exception {
		...
		ResultSet resultSet = statement.executeQuery("select license from licenses");

		String encryptedLicense = null;

		if (resultSet.next()) {
			encryptedLicense = resultSet.getString(1);
		}
